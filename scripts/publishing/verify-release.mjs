...
import { acquireProjectLockImmediate, acquireWorkspaceOutputLock, assertLiveProjectLock, assertLiveWorkspaceOutputLock, assertNoSymlinkPath, pinPhysicalDirectory } from "../state/project-lock.mjs";
...
export function verifyReleaseDirectory(directory, candidate, { manifest, root, heldLock = null, immutableRoot, workspaceRoot, heldWorkspaceLock = null } = {}) {
  if (workspaceRoot || heldWorkspaceLock) { if (!workspaceRoot || !heldWorkspaceLock) throw new Error("Workspace verification authority must include both root and live lock."); assertLiveWorkspaceOutputLock(heldWorkspaceLock, workspaceRoot); const logical = resolve(workspaceRoot), physical = pinPhysicalDirectory(workspaceRoot).path, physicalize = (path) => { const value = relative(logical, resolve(path)); if (value === ".." || value.startsWith(`..${sep}`)) throw new Error("Verification path escapes the physical workspace."); return resolve(physical, value); }; immutableRoot = physicalize(immutableRoot); directory = physicalize(directory); workspaceRoot = physical; assertNoSymlinkPath(physical, immutableRoot, { allowMissing: false }); }
