...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) { if (!process.argv[2] || !process.argv[3] || !process.argv[4]) throw new Error("Usage: pnpm release:verify -- <workspace-root> <book-project-path> <release-id>"); const workspaceRoot = resolve(process.argv[2]), project = resolveBookProject(process.argv[3], { workspaceRoot }), immutableRoot = resolve(workspaceRoot, "dist", "releases", "immutable"), directory = resolve(immutableRoot, project.id, process.argv[4]), workspaceLock = await acquireWorkspaceOutputLock(workspaceRoot, { ownerId: `release-verification-output-${process.pid}` }); try { const candidate = JSON.parse(readFileSync(resolve(directory, "candidate.json"), "utf8")), manifestFile = resolve(directory, "manifest.json"), manifest = existsSync(manifestFile) ? JSON.parse(readFileSync(manifestFile, "utf8")) : null; if (!manifest) throw new Error("The derived immutable release directory must contain manifest.json."); if (manifest.releaseId !== process.argv[4] || project.id !== candidate.projectId) throw new Error("Release identity does not match the requested project and release ID."); verifyReleaseDirectory(directory, candidate, { manifest, root: project.legacyRoot, immutableRoot, workspaceRoot, heldWorkspaceLock: workspaceLock }); console.log(`✓ Verified immutable release ${directory}`); } finally { workspaceLock.release(); } }
