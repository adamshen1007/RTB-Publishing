...
function assertSafeTree(directory) {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    else if (!stat.isFile()) throw new Error(`Release material must contain regular files only: ${entry.name}`);
  }
}
...
export function verifyReleaseDirectory(directory, candidate, { manifest, root, heldLock = null, immutableRoot } = {}) {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) { if (!process.argv[2] || !process.argv[3] || !process.argv[4]) throw new Error("Usage: pnpm release:verify -- dist/releases/immutable/<project-id>/<release-id> <book-project-path> <workspace-root>"); const directory = resolve(process.argv[2]), candidate = JSON.parse(readFileSync(resolve(directory, "candidate.json"), "utf8")), manifestFile = resolve(directory, "manifest.json"), manifest = existsSync(manifestFile) ? JSON.parse(readFileSync(manifestFile, "utf8")) : null, project = resolveBookProject(process.argv[3], { workspaceRoot: resolve(process.argv[4]) }); if (!manifest) throw new Error("The immutable release directory must contain manifest.json."); if (project.id !== candidate.projectId) throw new Error("Release project does not match the stored candidate."); const immutableRoot = resolve(directory, "..", ".."); verifyReleaseDirectory(directory, candidate, { manifest, root: project.legacyRoot, immutableRoot }); console.log(`✓ Verified immutable release ${directory}`); }
