...
export async function buildRelease(project, { lifecycleVersion = 0, approvalId = null, resourceFixture = null, resourceReport = null, buildRoot = resolve(BUILD_DIR, "releases"), outputRoot = resolve(DIST_DIR, "releases"), env = process.env } = {}) {
  ...
  const work = resolve(buildRoot, project.id), release = resolve(outputRoot, project.id);
  try {
    rmSync(work, { recursive: true, force: true }); rmSync(release, { recursive: true, force: true }); mkdirSync(work, { recursive: true }); mkdirSync(release, { recursive: true });
    ...
    let manifest = null;
    if (approvalId) { const finalized = await finalizeRelease({ root: project.legacyRoot, project, candidateHash: candidate.candidateHash, approvalId, releaseDirectory: release, heldLock: publicationLock }); manifest = finalized.manifest; releasePolicies = evaluateReleasePolicies(project, candidate); }
    verifyReleaseDirectory(release, candidate, { manifest, root: project.legacyRoot }); if (resourceReport) writeJson(resourceReport, resourceProof); return { project, snapshot, outputs, verification, candidate, releasePolicies, manifest, release };
