...
export function resolveCurrentGeneration(project, { outputRoot, hooks = {} }) {
  ...
  assertNoSymlinkPath(root, pointer, { allowMissing: false }); const pointerIdentity = pinPhysicalEntry(pointer, "file"), pointerBytes = readFileSync(pointer);
  let value;
  try { value = JSON.parse(pointerBytes.toString("utf8")); } catch { throw new Error(`Current build generation pointer is invalid for ${project.id}. Run pnpm build again.`); }
  ...
  assertNoSymlinkPath(root, generationRoot, { allowMissing: false }); const generationIdentity = pinPhysicalEntry(generationRoot, "directory"); assertPrivateTree(generationRoot); hooks.beforePointerRecheck?.({ pointer, pointerBytes, generationRoot }); assertPinnedEntry(pointerIdentity, "file"); const currentPointerBytes = readFileSync(pointer); if (!currentPointerBytes.equals(pointerBytes)) throw new Error(`Current build generation pointer changed while resolving ${project.id}; retry the request.`);
