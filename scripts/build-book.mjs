...
function retainGenerations(root, current, hooks) { const generations = collectGenerations(root, current), keep = new Set([current, ...generations.filter((item) => !item.current).slice(0, 2).map((item) => item.name)]); for (const item of generations) if (!keep.has(item.name)) { const identity = pinPhysicalEntry(item.path, "directory"); hooks.beforeGenerationGc?.({ generation: item.name, path: item.path }); assertPinnedEntry(identity, "directory"); rmSync(item.path, { recursive: true }); syncPath(root, "generation-gc-parent", hooks); hooks.afterGenerationGc?.({ generation: item.name, path: item.path }); } }
...
export function buildProject(project, { outputRoot = resolve(DIST_DIR, "books"), workspaceRoot = project.workspaceRoot ?? ROOT, hooks = {} } = {}) {
  ...
  let lock;
  const token = randomUUID(); let buildDirectory, outputDirectory, stagingGeneration, stagingBuild, stagingOutputRoot, stagingOutput, readyGeneration, pointerRenamed = false;
  try {
    ...
    if (stagingGeneration) rmSync(stagingGeneration, { recursive: true, force: true }); if (readyGeneration && !pointerRenamed) rmSync(readyGeneration, { recursive: true, force: true });
    throw error;
