...
import { discoverBookProject, resolveBookProject } from "./books/discovery.mjs";
...
function parseSources(project) {
  if (!project.manifest.paths.research) return [];
...
export function publicationExport(projectOrRoot) {
  const project = typeof projectOrRoot === "object" && projectOrRoot.chapters ? projectOrRoot : projectOrRoot ? discoverBookProject(projectOrRoot) : resolveBookProject();
...
export function validatePublicationExport(payload) {
  ...
  for (const chapter of payload.chapters) { if (!chapter.part) failures.push(`${chapter.id}: missing part`); if (!chapter.reader_decision || !chapter.required_output) failures.push(`${chapter.id}: missing reader decision or output`); if (!/^[a-f0-9]{64}$/.test(chapter.sourceHash)) failures.push(`${chapter.id}: invalid source hash`); }
  return failures;
}
...
if (resolve(process.argv[1] ?? "") === fileURLToPath(import.meta.url)) {
  const command = process.argv[2] ?? "check"; const project = process.argv[3]; const payload = publicationExport(project); const failures = validatePublicationExport(payload);
