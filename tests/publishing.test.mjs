...
test("beginner release docs require copying an exact command and contain no executable release placeholder", () => {
  for (const file of ["docs/05-operations/beginner-publication-approval.md", "docs/05-operations/verified-release-candidate.md"]) {
    const markdown = readFileSync(resolve(file), "utf8");
    assert.match(markdown, /copy (?:the )?(?:complete )?(?:shell-quoted )?verification command|Copy that command exactly/i);
    assert.doesNotMatch(markdown, /pnpm release:verify[^\n]*(?:\n[^\n]*){0,5}<release-id>/);
  }
});
